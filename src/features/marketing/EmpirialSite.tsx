...
const Logo = () => (
  <Link to="/" className="flex items-center gap-3" aria-label="EMPIRIAL home">
    <BrandIcon size={36} />
    <span className="text-sm font-semibold tracking-[.36em] text-[#191521] dark:text-white">EMPIRIAL</span>
  </Link>
);
...
export const SiteNavigation = () => {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-6">
    ...
    <nav className="mx-auto grid max-w-7xl grid-cols-[1fr_auto_1fr] items-center gap-4 rounded-2xl border border-black/10 dark:border-white/10 bg-[#fdfbf8]/85 dark:bg-[#07070c]/80 px-5 py-4 shadow-2xl backdrop-blur-xl">
      <div className="col-start-1 justify-self-start"><Logo /></div>
      <div className="col-start-2 hidden items-center gap-7 md:flex">
        {items.map(([label, href]) => <Link key={href} to={href} className={`text-sm transition ${location.pathname === href ? 'text-[#191521] dark:text-white' : 'text-[#191521]/55 dark:text-white/55 hover:text-[#191521] dark:hover:text-white'}`}>{label}</Link>)}
      </div>
      <div className="col-start-3 flex items-center justify-self-end gap-3">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <Link to="/staff" className="rounded-full border border-black/15 dark:border-white/15 px-4 py-2.5 text-sm font-semibold text-[#191521]/80 dark:text-white/80 transition hover:border-black/30 dark:hover:border-white/30 hover:text-[#191521] dark:hover:text-white">Staff</Link>
        ...
        <button className="text-[#191521] dark:text-white md:hidden" onClick={() => setOpen(!open)} aria-label="Toggle menu">{open ? <X /> : <Menu />}</button>
      </div>
      {open && <div className="absolute inset-x-3 top-16 rounded-2xl border border-black/10 dark:border-white/10 bg-[#fdfbf8] dark:bg-[#0d0d15] p-4 md:hidden">{items.map(([label, href]) => <Link onClick={() => setOpen(false)} key={href} to={href} className="block border-b border-black/5 dark:border-white/5 px-3 py-3 text-[#191521]/75 dark:text-white/75 last:border-0">{label}</Link>)}<Link onClick={() => setOpen(false)} to="/staff" className="mt-3 block rounded-full border border-black/15 dark:border-white/15 px-3 py-3 text-center font-semibold text-[#191521]/80 dark:text-white/80">Staff</Link><Link onClick={() => setOpen(false)} to="/auth" className="mt-3 block rounded-full bg-[#8138ff] px-3 py-3 text-center font-semibold text-white">Sign In</Link></div>}
    </nav>
  </header>;
};

export const SiteFooter = () => <footer className="border-t border-black/10 dark:border-white/10 bg-[#f5f1eb] dark:bg-[#050508] px-6 py-14">
  <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-[1.5fr_1fr_1fr_1.2fr]">
    <div><Logo /><p className="mt-5 max-w-xs text-sm leading-6 text-[#191521]/45 dark:text-white/45">Intelligent digital solutions that move ambitious businesses forward.</p></div>
    <div><p className="mb-4 text-xs uppercase tracking-[.2em] text-[#191521]/40 dark:text-white/40">Explore</p><div className="grid gap-3 text-sm text-[#191521]/65 dark:text-white/65"><Link to="/services">Services</Link><Link to="/portfolio">Portfolio</Link><Link to="/about">About</Link><Link to="/contact">Contact</Link></div></div>
    <div><p className="mb-4 text-xs uppercase tracking-[.2em] text-[#191521]/40 dark:text-white/40">Capabilities</p><div className="grid gap-3 text-sm text-[#191521]/65 dark:text-white/65"><span>Web & apps</span><span>AI automation</span><span>Brand systems</span><span>Growth</span></div></div>
    <div><p className="mb-4 text-xs uppercase tracking-[.2em] text-[#191521]/40 dark:text-white/40">Start a conversation</p><p className="text-sm text-[#191521]/70 dark:text-white/70">hello@empirialdesigns.com</p><p className="mt-2 text-sm text-[#191521]/45 dark:text-white/45">Polokwane, South Africa</p><a href="https://wa.me/27651859143" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-[#7c2cff] dark:text-[#b56cff]">WhatsApp us <ArrowRight className="h-4 w-4" /></a></div>
  </div>
  <div className="mx-auto mt-12 max-w-7xl border-t border-black/10 dark:border-white/10 pt-6 text-xs text-[#191521]/35 dark:text-white/35">© 2026 EMPIRIAL. All rights reserved.</div>
</footer>;

const Page = ({ children }: { children: ReactNode }) => {
  const isDark = useSystemTheme();
  return <div className={isDark ? 'dark' : ''}><div className="min-h-screen bg-[#f5f1eb] text-[#191521] transition-colors duration-500 dark:bg-[#050508] dark:text-white"><SiteNavigation /><main>{children}</main><SiteFooter /></div></div>;
};
const Eyebrow = ({ children }: { children: ReactNode }) => <p className="mb-4 text-xs font-semibold uppercase tracking-[.24em] text-[#7c2cff] dark:text-[#b56cff]">{children}</p>;
...
const Process = () => <section className="mx-auto max-w-7xl px-6 py-24"><Eyebrow>Our approach</Eyebrow><div className="grid gap-10 md:grid-cols-[.8fr_1.2fr]"><h2 className="text-4xl font-semibold tracking-tight md:text-5xl">Strategy. Design. Build. <span className={purple}>Scale.</span></h2><div className="grid gap-5 sm:grid-cols-4">{[['01','Discover','We learn your goals, audience, and opportunity.'],['02','Design','We shape the right experience and visual system.'],['03','Build','We create clean, fast, robust digital products.'],['04','Scale','We launch, optimise, and help you grow.']].map(([n,t,d]) => <div key={n} className="border-t border-black/15 dark:border-white/15 pt-4"><span className="text-sm text-[#7c2cff] dark:text-[#b56cff]">{n}</span><h3 className="mt-5 font-semibold">{t}</h3><p className="mt-2 text-sm leading-6 text-[#191521]/45 dark:text-white/45">{d}</p></div>)}</div></div></section>;

export const ServicesPage = () => <Page><section className="px-6 pb-20 pt-36"><div className="mx-auto max-w-7xl"><Eyebrow>Capabilities</Eyebrow><h1 className="max-w-4xl text-5xl font-semibold tracking-tight sm:text-7xl">Digital solutions built for <span className={purple}>ambitious businesses.</span></h1><p className="mt-7 max-w-2xl text-lg leading-8 text-[#191521]/55 dark:text-white/55">From your first landing page to the systems that run your growth, EMPIRIAL brings strategy, design, and technology together.</p></div></section><section className="mx-auto max-w-7xl px-6 pb-24"><div className="grid gap-4">{services.map(({icon: Icon,title,text},i) => <div key={title} className="grid gap-6 rounded-2xl border border-black/10 dark:border-white/10 bg-black/[.03] dark:bg-white/[.03] p-7 md:grid-cols-[80px_1fr_auto] md:items-center"><div className="grid h-14 w-14 place-items-center rounded-xl bg-[#7c2cff]/15 text-[#7c2cff] dark:text-[#b56cff]"><Icon /></div><div><p className="mb-2 text-xs text-[#7c2cff] dark:text-[#b56cff]">0{i+1}</p><h2 className="text-2xl font-semibold">{title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#191521]/50 dark:text-white/50">{text}</p></div><Link to="/contact" className="text-sm font-semibold text-[#191521]/70 dark:text-white/70 hover:text-[#191521] dark:hover:text-white">Discuss this <ArrowRight className="ml-1 inline h-4 w-4" /></Link></div>)}</div></section><Process /></Page>;
...
export const PortfolioReferencePage = () => {
  ...
  return <Page>
    <section className="px-6 pb-8 pt-32 text-center sm:pt-36"><div className="mx-auto max-w-2xl"><Eyebrow>Our work</Eyebrow><h1 className="text-4xl font-semibold tracking-tight sm:text-6xl">Built to be seen. <span className={purple}>Made to move.</span></h1><p className="mx-auto mt-5 max-w-lg text-sm leading-6 text-[#191521]/55 dark:text-white/55 sm:text-base">A visual diary of the brands, products, and systems we have helped bring to life.</p></div></section>
    <section className="mx-auto max-w-6xl px-6 pb-7"><div className="flex flex-wrap justify-center gap-2" aria-label="Filter portfolio projects">{filters.map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded-full border px-4 py-2 text-xs font-medium transition sm:px-5 sm:py-2.5 sm:text-sm ${filter === item ? 'border-[#8138ff] bg-[#8138ff] text-white shadow-[0_0_20px_rgba(129,56,255,.25)]' : 'border-black/20 dark:border-white/20 text-[#191521]/60 dark:text-white/60 hover:border-black/45 dark:hover:border-white/45 hover:bg-black/5 dark:hover:bg-white/5 hover:text-[#191521] dark:hover:text-white'}`}>{item}</button>)}</div></section>
    <section className="pb-24"><PortfolioCoverflow projects={visibleProjects} /></section>
    <section className="mx-auto max-w-7xl px-6 pb-24"><div className="rounded-2xl border border-dashed border-black/15 dark:border-white/15 p-8 text-center"><Sparkles className="mx-auto mb-4 text-[#7c2cff] dark:text-[#b56cff]" /><p className="text-sm text-[#191521]/50 dark:text-white/50">More work is coming as the EMPIRIAL family grows.</p><p className="mt-2 text-xl font-semibold">Your project could be next.</p></div></section>
  </Page>;
};

export const AboutPage = () => <Page><section className="px-6 pb-24 pt-36"><div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.1fr_.9fr]"><div><Eyebrow>About EMPIRIAL</Eyebrow><h1 className="text-5xl font-semibold tracking-tight sm:text-7xl">Building intelligent digital <span className={purple}>businesses.</span></h1><p className="mt-7 max-w-xl text-lg leading-8 text-[#191521]/55 dark:text-white/55">EMPIRIAL exists to help ambitious people turn good ideas into clear, credible, useful digital experiences.</p></div><img src={humanRobot} alt="Human and EMPIRIAL robot collaborating" className="aspect-[4/3] w-full max-w-xl rounded-2xl object-cover" /></div></section><section className="mx-auto grid max-w-7xl gap-12 border-t border-black/10 dark:border-white/10 px-6 py-24 lg:grid-cols-2"><div><Eyebrow>Your story, made digital</Eyebrow><h2 className="text-4xl font-semibold">The best work sits where <span className={purple}>human thinking</span> meets useful technology.</h2></div><div className="space-y-5 text-[#191521]/55 dark:text-white/55"><p className="leading-8">We started EMPIRIAL because too many businesses are forced to choose between beautiful design and technology that works. We believe the best digital products need both.</p><p className="leading-8">Our job is to bring clarity to complex ideas, create experiences people trust, and build systems that help your business keep moving after launch.</p></div></section><section className="mx-auto max-w-7xl px-6 pb-24"><img src={workspace} alt="EMPIRIAL futuristic workspace" className="mb-8 h-72 w-full rounded-2xl object-cover opacity-80" /><div className="grid gap-4 md:grid-cols-4">{[['01','Ideas into reality.'],['02','Human + AI connection.'],['03','Signal of intelligence.'],['04','Forward momentum.']].map(([n,t]) => <div key={n} className="rounded-2xl border border-black/10 dark:border-white/10 p-6"><p className="text-sm text-[#7c2cff] dark:text-[#b56cff]">{n}</p><p className="mt-14 text-lg font-semibold">{t}</p></div>)}</div></section></Page>;
