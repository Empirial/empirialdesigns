...
import {
  ...
  Search, Settings, Sparkles, Trash2, TrendingUp, Upload, UserRound, Users,
...
const nav = [
  ...
  // Growth itself (SEO, PageSpeed, Search Console, uptime, reviews, Business
  // Profile, domain) is a per-project page at /dashboard/growth/:repoId,
  // opened from inside the builder's own "Growth" button — see
  // BuilderPage.tsx and Growth.tsx's own header comment for why it stays a
  // standalone page rather than living inside this sidebar shell. This nav
  // entry's bare /dashboard/growth (no id) is the picker below (Screen's
  // isGrowthPicker branch) — the only thing the sidebar itself owns here is
  // getting you to a project to open Growth for.
  { label: 'Growth', icon: TrendingUp, path: '/dashboard/growth' },
...
function Screen({ path, projects, search, setSearch, filter, setFilter, navigate, create, showNotice, userId, onProjectsChanged }: ScreenProps) { const isTemplates = path.endsWith('/templates'); const isAssets = path.endsWith('/assets'); const isGrowthPicker = path === '/dashboard/growth'; const [connected, setConnected] = useState<Record<string, boolean>>({});
  ...
  if (isGrowthPicker) return <div className="page-wide"><PageIntro icon={TrendingUp} title="Growth" subtitle="Pick a project to see its search, speed, uptime, reviews, and domain."/><div className="filter-bar"><Search size={16}/><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search projects..."/><div className="chip-row ml-auto">{['All','Website','Image','Document'].map(x => <button className={filter === x ? 'active' : ''} key={x} onClick={() => setFilter(x)}>{x === 'Website' ? 'Websites' : x + (x === 'All' ? '' : 's')}</button>)}</div></div>{projects.length ? <div className="project-grid">{projects.map((p: Project) => <div className="project-card" key={p.id}><div className="project-card-image" style={{background:p.color}}>{p.image && <img src={p.image} alt=""/>}<span className="project-type">{p.type}</span></div><div className="p-4 flex items-center"><div><h3 className="text-sm font-semibold">{p.name}</h3><p className="mt-1 text-[11px] text-white/35">Updated {p.updated}</p></div><button onClick={() => navigate(`/dashboard/growth/${p.id}`)} className="ml-auto icon-button" aria-label={`Open Growth for ${p.name}`}><TrendingUp size={16}/></button></div></div>)}</div> : <div className="empty-state"><TrendingUp size={28}/><h3>No projects found</h3><p>Try a different search, or start a new project first — Growth needs a published site to work with.</p></div>}</div>;
